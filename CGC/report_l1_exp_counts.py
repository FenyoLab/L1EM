...
for name in names:
	if 'exon' not in name:
		...
		print_string += '\t'+str(total*X_est[runthroughS_name]/proper_pairs_in_original_bam*10**6)
		runthroughA_name = seq_name+'_antisenserunthrough'
		if runthroughA_name not in X_est:
			X_est[runthroughA_name]=0.0
		print_string += '\t'+str(total*X_est[runthroughA_name]/proper_pairs_in_original_bam*10**6)

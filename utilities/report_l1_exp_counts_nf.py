...
# Changes compared to the original: script now needs the input file to be passed as argument
...
X_est = dict(zip(pickle.load(open(sys.argv[2],'rb')),pickle.load(open(sys.argv[3],'rb'))))
...
for name in names:
	if 'exon' not in name:
		...
		print_string += '\t'+str(total*X_est[senserunthrough_name])
		...
		print_string += '\t'+str(total*X_est[antisenserunthrough_name])

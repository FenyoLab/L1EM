...
L1EM_bed=$L1EM_directory'/annotation/mm39.L1EM.bed'
L1EM_fa=$L1EM_directory'/annotation/mm39.L1EM.400.fa'
...
cd ..
mkdir split_fqs
split_fq_size=$(wc -l idL1reads/L1.fq1 | awk '{print $1/('$threads'*10*4)+1}' | cut -d '.' -f 1 | awk '{print $1*4}')
...
for name in *.fq1.*
    ...
    samtools view -@ $threads -bS temp.$base.aln.sam > temp.$base.aln.bam
    samtools sort -@ $threads -n temp.$base.aln.bam > $base.aln.bam
...
ls ../split_fqs/*.bam > list_of_bams.txt
split -l $threads list_of_bams.txt list_of_bams.txt.
for bamlist in list_of_bams.txt.*
	do for bam in $(cat $bamlist)
...
$python ${L1EM_utilities_dir}filtered_and_normalized_active_l1md_unstranded.py names_final.pkl X_final.pkl $(head -2 ../baminfo.txt | tail -1) $(head -3 ../baminfo.txt | tail -1)> ../filter_active_L1Md_FPM.txt

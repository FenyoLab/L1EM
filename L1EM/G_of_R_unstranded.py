			...
			for aln2 in alignments2.alignments[refname]:
				...
				if max(aln1.start,aln2.start)-min(aln1.start,aln2.start) <= max_start2start_len:
					...
					if  within_5p and within_3p and has_5pUTR:
						this_G_of_R[1*nreps+rnames_index[refname]] += aln1.P*aln2.P/max(rlens[rnames_index[refname]]-2*flanking-insert_mean+2*wiggle,min_len)
					if  within_5p and has_5pUTR:
						this_G_of_R[2*nreps+rnames_index[refname]] += aln1.P*aln2.P/(rlens[rnames_index[refname]]-2*flanking+2*wiggle)

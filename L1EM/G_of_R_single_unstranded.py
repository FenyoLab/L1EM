...
def GetArgs():
	...
	return args.bamfile, args.error_prob, args.reads_per_pickle, args.prefix, args.NMdiff, args.flanking, args.wiggle, args.min_len, args.min_exon_len
...
"""
def make_G_of_R_row(alignments,rnames_index,rlens,nreps,read_length,flanking,wiggle,min_len,min_exon_len):
	this_G_of_R = numpy.zeros(3*nreps)
	for refname in alignments.alignments:
		for aln in alignments.alignments[refname]:
			has_5pUTR = refname.split('.')[1]=='1'
			within_5p = aln.start > flanking -wiggle
			within_3p = aln.start+read_length < rlens[rnames_index[refname]]-flanking +wiggle
			overlap_element = aln.start+read_length > flanking and aln.start < rlens[rnames_index[refname]]-flanking
			if not overlap_element:
				continue
			this_G_of_R[rnames_index[refname]] += aln.P/(rlens[rnames_index[refname]]-2*flanking+read_length+2*wiggle)
			if  within_5p and within_3p and has_5pUTR:
				this_G_of_R[1*nreps+rnames_index[refname]] += aln.P/max(rlens[rnames_index[refname]]-2*flanking-read_length+2*wiggle,min_len)
			if  within_5p and has_5pUTR:
				this_G_of_R[2*nreps+rnames_index[refname]] += aln.P/(rlens[rnames_index[refname]]-2*flanking+2*wiggle)
...
def main():
	...
	for i in range(nreps):
		TEnamefile.write(rnames[i]+'_runthrough'+'\t'+str(rlens[i]+2*flanking)+'\n')
	for i in range(nreps):
